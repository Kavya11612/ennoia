...
const inputClasses =
  'w-full min-h-[48px] bg-card border border-rule rounded-sm px-3 py-3 font-sans text-[16px] text-ink caret-ink focus:border-ink focus:ring-2 focus:ring-ink focus:ring-offset-2 outline-none transition-colors duration-fast ease-out';
...
export default function ContactForm({
  ...
  return (
    <form method="POST" action="/api/contact" className="relative z-10 flex flex-col gap-5">
      {/* Honeypot */}
      ...
      <Field label="What are you trying to solve?" htmlFor="problem" error={errors.problem}>
        <textarea id="problem" name="problem" rows={5} required className={inputClasses} />
      </Field>

      <Field label="What kind of help?" htmlFor="help">
        <select id="help" name="help" multiple className={`${inputClasses} h-auto`} size={helpOptions.length}>
          {helpOptions.map((opt) => (
      ...
      <button
        type="submit"
        className="self-start inline-flex items-center bg-ink text-open-board px-6 py-3 rounded-md font-sans font-semibold text-[14px] hover:bg-ink-hover transition-colors duration-fast ease-out min-h-[44px]"
      >
        Send
      </button>

      <p className="font-sans text-body-s text-gambit-text">
        Or email{' '}
