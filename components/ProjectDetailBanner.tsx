...
export default function ProjectDetailBanner({
  ...
  return (
    <div
      className={`relative overflow-hidden rounded-md ${surface.shell} min-h-[220px] md:min-h-[320px]`}
    >
      <div className="pointer-events-none absolute inset-0 overflow-hidden" aria-hidden="true">
        {surface.deco}
      </div>

      <div className="relative z-[1] flex h-full min-h-[220px] md:min-h-[320px] w-full flex-col justify-end gap-4 p-5 md:p-7 lg:p-8">
        <div className="flex min-w-0 flex-col gap-2 md:gap-3">
          {project.projectType === 'hypothetical' && (
            <span className={`font-mono text-[12px] tracking-[0.08em] ${surface.mute}`}>
              Hypothetical
            </span>
          )}
          <h1 className={`font-sans font-bold text-h1 max-w-[16ch] text-balance ${surface.ink}`}>
            {project.brandName}
          </h1>
          <p className={`font-sans text-body-l max-w-[42ch] ${surface.mute}`}>{project.oneLiner}</p>
          <p className={`font-mono text-meta tracking-[0.06em] ${surface.mute}`}>
