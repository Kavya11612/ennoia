...
function ArchImage({ src }: { src: string }) {
  return (
    <div
      className="relative aspect-[4/5] w-[min(100%,7.75rem)] sm:w-[8rem] md:w-[clamp(7.25rem,14.5vh,9rem)] overflow-hidden bg-landing-stone shadow-[0_12px_28px_-20px_rgba(40,32,24,0.4)]"
      style={{ borderRadius: '999px 999px 8px 8px' }}
    >
      <Image src={src} alt="" fill sizes="160px" className="object-cover object-center" />
    </div>
  );
}

/** Home H4 — What we do zigzag practices (mockup). */
export default function HomePractices({ practices }: { practices: Practice[] }) {
  return (
    <div>
      ...
      <ul className="mt-7 md:mt-8 flex flex-col gap-6 md:gap-[clamp(0.9rem,2vh,1.35rem)]">
        {practices.map((practice, i) => {
          ...
          return (
            <li key={practice.slug}>
              <Link
                href="/studio"
                className={`flex flex-col gap-4 md:items-center md:gap-0 group ${
                  imageLeft ? 'md:flex-row' : 'md:flex-row-reverse'
                }`}
              >
                <div className={`flex md:flex-1 ${imageLeft ? 'md:justify-end' : 'md:justify-start'}`}>
                  {src ? <ArchImage src={src} /> : null}
                </div>

                <div className="hidden md:flex items-center shrink-0 px-1.5 lg:px-3 min-w-[5.5rem]">
                  <span className="h-px w-6 lg:w-9 bg-rule" aria-hidden="true" />
                  <span className="inline-flex h-9 w-9 shrink-0 items-center justify-center rounded-full bg-ink font-mono text-[11px] tracking-[0.06em] text-open-board tabular-nums">
                ...
                <div
                  className={`flex flex-1 items-start md:items-center gap-3 md:gap-0 ${
                    imageLeft ? 'md:justify-start' : 'md:justify-end'
                  }`}
                >
                  <span className="md:hidden inline-flex h-9 w-9 shrink-0 items-center justify-center rounded-full bg-ink font-mono text-[11px] tracking-[0.06em] text-open-board tabular-nums">
                    {String(n).padStart(2, '0')}
                  </span>
                  <div
                    className={`flex flex-col gap-1.5 max-w-[22rem] ${
                      imageLeft ? 'md:items-start md:text-left' : 'md:items-end md:text-right'
                    }`}
                  >
                    <span className="inline-flex h-7 w-7 items-center justify-center rounded-full border border-rule bg-card text-emboss">
                      {practiceIcons[practice.slug]}
                    </span>
                    <h3 className="font-sans font-semibold text-[1.06rem] md:text-[1.12rem] leading-snug text-ink text-balance group-hover:text-emboss-link transition-colors duration-fast ease-out">
                      {practice.name}
                    </h3>
                    <p className="font-sans text-[0.9rem] md:text-[0.94rem] leading-snug text-gambit-text">
                      {practice.oneLiner}
                    </p>
                  </div>
                </div>
